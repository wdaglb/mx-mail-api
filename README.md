...
如果新增的是子域名，例如 `edss.bbroot.com`，服务会按这个子域名本身查询 DNS。也就是说，需要确保 `edss.bbroot.com` 这个域名能查到自己的解析结果；服务不会自动退到 `bbroot.com` 或更上级域名代查。
...
如果验证的是子域名，还需要确认子域名自身可被查询。例如新增 `edss.bbroot.com` 时，服务会按 `edss.bbroot.com` 查询，不会把查询边界自动放宽到 `bbroot.com`。
